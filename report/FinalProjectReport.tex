...
Hệ thống tuân theo \textbf{Kiến trúc Microservices} với các thành phần sau:

\begin{itemize}
    \item \textbf{API Gateway (Cổng 8888):} Điểm vào duy nhất, điều hướng, xác thực JWT
    \item \textbf{Dịch vụ Identity (Cổng 3001):} Xác thực, quản lý người dùng, RBAC
    \item \textbf{Dịch vụ Profile (Cổng 3002):} Quản lý hồ sơ người dùng và tenant
    \item \textbf{Dịch vụ Product (Cổng 3003):} Danh mục menu, món ăn, modifiers
    \item \textbf{Dịch vụ Table (Cổng 3004):} CRUD bàn, tạo mã QR
    \item \textbf{Dịch vụ Order (Cổng 3005):} Vòng đời đơn hàng, state machine
    \item \textbf{Dịch vụ Kitchen (Cổng 3008):} Chức năng KDS
    \item \textbf{Dịch vụ Waiter (Cổng 3009):} Chấp nhận/từ chối đơn hàng
    \item \textbf{Dịch vụ Notification (Cổng 3007):} Cập nhật WebSocket thời gian thực
\end{itemize}
...
\begin{lstlisting}[language=bash]
# Sử dụng tệp batch được cung cấp
./run.bat

# Hoặc khởi động thủ công từng dịch vụ
cd src/backend/api-gateway && npm run start:dev
cd src/backend/identity && npm run start:dev
# ... khởi động các dịch vụ khác
...
API Gateway & 8888 \\
\hline
Dịch vụ Identity & 3001 \\
\hline
Dịch vụ Profile & 3002 \\
\hline
Dịch vụ Product & 3003 \\
\hline
Dịch vụ Table & 3004 \\
\hline
Dịch vụ Order & 3005 \\
\hline
Dịch vụ Notification & 3007 \\
\hline
Dịch vụ Kitchen & 3008 \\
\hline
Dịch vụ Waiter & 3009 \\
\hline
\end{tabular}
...
\begin{itemize}
    \item Kiến trúc microservices hoàn chỉnh với 9 dịch vụ backend
    \item Đặt món dựa trên QR bảo mật với token đã ký
    \item Cập nhật thời gian thực qua WebSocket

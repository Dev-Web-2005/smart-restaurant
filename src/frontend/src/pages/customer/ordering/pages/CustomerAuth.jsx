...
const CustomerAuth = ({ onClose, onSuccess, tenantId: propTenantId }) => {
	...
	const handleForgotPassword = async (e) => {
		...
		try {
			...
			console.log('🔍 Checking email verification status for:', forgotEmail)
			const verificationResult = await checkEmailVerificationStatusAPI(forgotEmail)
			...
			if (!verificationResult.isVerified) {
				setErrorMessage(
					'⚠️ Your email is not verified. Please verify your email address before requesting a password reset. You can verify your email from your profile settings.',
				)
				setLoading(false)
				return
			}
						...
						{activeTab === 'login' && (
							...
							>
								...
								<button
									...
									onClick={() => {
										if (onSuccess) {
											onSuccess({ isGuest: true })
										}
									}}
									className="flex h-12 w-full items-center justify-center gap-2 rounded-lg bg-gray-700 text-base font-semibold text-white transition-all hover:bg-gray-600 border-none cursor-pointer mt-3"
									disabled={loading}
								>

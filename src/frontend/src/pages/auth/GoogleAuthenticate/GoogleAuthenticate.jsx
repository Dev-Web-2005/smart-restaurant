		...
		const handleGoogleCallback = async () => {
			...
			try {
				...
				if (response.status === 200 && response.data && response.data.code === 1000) {
					...
					// ✅ Store customer data exactly like customerLoginAPI does
					const customerData = {
						userId,
						username,
						email,
						...
						accessToken, // ✅ Include accessToken for persistence (same as customerLoginAPI)
					}
					...
					// Google login now directly logs in user without requiring password setup
					// Account is auto-created in backend with random password, login is based on email/Google info only
					if (finalOwnerId) {
						if (tableNumber) {
	...
	return (
		<div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-black flex items-center justify-center">
			<div className="text-center">
				...
				{/* Loading Text */}
				<h2 className="text-white text-2xl font-semibold mb-4">Đang xác thực</h2>
				...
				{/* Sub text */}
				<p className="text-gray-400 text-sm mt-6">Vui lòng chờ trong giây lát...</p>
			</div>
		</div>
	)
}
